...
const LipidAdmin: React.FC = () => {
  ...
  const handleViewPdf = async (record: LipidRecord) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/download-pdf/${encodeURIComponent(
          record.fileName
        )}`,
        {
          method: "GET",
  ...
  const handleFileUpload = async (
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          const response = await fetch(
            "http://localhost:8000/extract-lipid-profile",
            {
              method: "POST",
              body: formData,
            }
  ...
  const renderLabValue = (label: string, value: LipidValue | undefined) => {
    ...
    const flagStyle = value.flag
      ? { color: getFlagColor(value.flag), fontWeight: "bold" as const }
      : {};
          ...
          {value.result ? (
            <>
              ...
              {value.reference_range && (
                <span> | Ref: {value.reference_range}</span>
              )}
            </>
          ) : (
            "N/A"
          ...
          <section className={styles.uploadSection}>
            <div className={styles.sectionHeader}>
              ...
              <p className={styles.sectionDescription}>
                Select PDF files containing Lipid Profile Results (Max 10MB per
                file)
              </p>
            </div>
                    ...
                    <div className={styles.recordHeader}>
                      ...
                      <div className={styles.recordActions}>
                        ...
                        <button
                          ...
                          style={{
                            backgroundColor: record.fileName
                              ? "#28a745"
                              : "#6c757d",
                            color: "white",
                            margin: "0 2px",
                            cursor:
                              record.fileName && !loading
                                ? "pointer"
                                : "not-allowed",
                          }}
                        >
                    ...
                    <div className={styles.recordDetails}>
                      ...
                      <div className={styles.recordItem}>
                        ...
                        <span
                          ...
                          style={{
                            color: getFlagColor(record.total_cholesterol?.flag),
                          }}
                        >
                          {record.total_cholesterol?.result || "N/A"}
                          {record.total_cholesterol?.flag &&
                            ` (${record.total_cholesterol.flag})`}
                        </span>
                      </div>
                      <div className={styles.recordItem}>
                        ...
                        <span
                          className={styles.recordValue}
                          style={{
                            color: getFlagColor(record.hdl_cholesterol?.flag),
                          }}
                        >
                          {record.hdl_cholesterol?.result || "N/A"}
                          {record.hdl_cholesterol?.flag &&
                            ` (${record.hdl_cholesterol.flag})`}
                        </span>
                      </div>
                      <div className={styles.recordItem}>
                        ...
                        <span
                          className={styles.recordValue}
                          style={{
                            color: getFlagColor(record.ldl_cholesterol?.flag),
                          }}
                        >
                          {record.ldl_cholesterol?.result || "N/A"}
                          {record.ldl_cholesterol?.flag &&
                            ` (${record.ldl_cholesterol.flag})`}
                        </span>
                      </div>
                    </div>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalContent}>
              ...
              <div className={styles.examSection}>
                ...
                {renderLabValue(
                  "Total Cholesterol",
                  selectedRecord.total_cholesterol
                )}
                {renderLabValue("Triglycerides", selectedRecord.triglycerides)}
                {renderLabValue(
                  "HDL Cholesterol",
                  selectedRecord.hdl_cholesterol
                )}
                {renderLabValue(
                  "LDL Cholesterol",
                  selectedRecord.ldl_cholesterol
                )}
                    ...
                    <div className={styles.riskItem}>
                      ...
                      <span className={styles.riskValue}>
                        {selectedRecord.hdl_cholesterol?.result
                          ? parseFloat(selectedRecord.hdl_cholesterol.result) >
                            75
                            ? "High (Good)"
                            : parseFloat(
                                selectedRecord.hdl_cholesterol.result
                    ...
                    <div className={styles.riskItem}>
                      ...
                      <span className={styles.riskValue}>
                        {selectedRecord.ldl_cholesterol?.result
                          ? parseFloat(selectedRecord.ldl_cholesterol.result) >
                            130
                            ? "High Risk"
                            : parseFloat(
                                selectedRecord.ldl_cholesterol.result
